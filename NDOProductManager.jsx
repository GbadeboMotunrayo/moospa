...
export default function NDOProductManager() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Segoe UI', Arial, sans-serif", background: "#f8f8f8", position: "relative" }}>
      ...
      <div style={{ width: 220, background: "#1a1a1a", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <div style={{ padding: 20, borderTop: "1px solid #333" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ color: "#888", fontSize: 12 }}>{connected ? "Connected to website" : "Offline"}</span>
          </div>
          <button
      ...
      <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "16px 24px", background: "white", borderBottom: "1px solid #eee", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>House of NDO — Admin Panel</div>
          </div>
          {activeTab === "products" && (
            <button
        ...
        <div style={{ padding: 24, flex: 1 }}>
          ...
          {activeTab === "dashboard" && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 32 }}>
                {[
                  { label: "Total Products", value: stats.total, color: PINK, bg: LIGHT_PINK },
                  { label: "In Stock", value: stats.inStock, color: "#2e7d32", bg: "#e8f5e9" },
                  { label: "Low Stock", value: stats.lowStock, color: "#f57f17", bg: "#fff8e1" },
                  { label: "Out of Stock", value: stats.outOfStock, color: "#c62828", bg: "#fce4ec" },
                ].map(card => (
              ...
              <div style={{ background: "white", borderRadius: 12, padding: 24, border: "1px solid #eee" }}>
                <div style={{ fontSize: 16, fontWeight: "bold", marginBottom: 16, color: "#1a1a1a" }}>Recent Products</div>
                {products.slice(0, 4).map(p => {
                  ...
                  return (
                    <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 0", borderBottom: "1px solid #f5f5f5" }}>
                      ...
                      <div style={{ fontWeight: "bold", color: PINK }}>₦{p.price.toLocaleString()}</div>
                  ...
                  <thead>
                    <tr style={{ background: "#f9f9f9" }}>
                      {["Product", "Category", "Price", "Stock", "Actions"].map(h => (
                        <th key={h} style={{ padding: "14px 20px", textAlign: "left", fontSize: 12, color: "#888", fontWeight: "bold", letterSpacing: 1, borderBottom: "1px solid #eee" }}>{h.toUpperCase()}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map(p => {
                      ...
                      return (
                        <tr key={p.id} style={{ borderBottom: "1px solid #f5f5f5" }}>
                          ...
                          <td style={{ padding: "14px 20px", fontWeight: "bold", color: PINK, fontSize: 14 }}>₦{p.price.toLocaleString()}</td>
                          <td style={{ padding: "14px 20px" }}>
          ...
          {activeTab === "categories" && (
            <div style={{ background: "white", borderRadius: 12, padding: 24, border: "1px solid #eee" }}>
              <div style={{ fontSize: 16, fontWeight: "bold", marginBottom: 20, color: "#1a1a1a" }}>Product Categories</div>
              {["Skincare", "Sex Toys", "Accessories", "Spa Services"].map(cat => (
                <div key={cat} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderRadius: 8, background: "#f9f9f9", marginBottom: 12 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>{products.filter(p => p.category === cat).length} products</div>
                  </div>
                  <button style={{ padding: "6px 14px", background: LIGHT_PINK, border: `1px solid ${PINK}44`, borderRadius: 6, color: PINK, fontSize: 12, fontWeight: "bold", cursor: "pointer" }}>Rename</button>
                </div>
              ))}
            </div>
          )}
          ...
          {activeTab === "settings" && (
            <div style={{ background: "white", borderRadius: 12, padding: 24, border: "1px solid #eee" }}>
              <div style={{ fontSize: 16, fontWeight: "bold", marginBottom: 20, color: "#1a1a1a" }}>App Settings</div>
              {[
                { label: "Website API URL", value: "https://houseofndo.com/api", editable: true },
                { label: "Admin Email", value: "admin@ndo.com", editable: true },
                { label: "Cloudinary Bucket", value: "ndo-products", editable: false },
                { label: "App Version", value: "v1.0.0", editable: false },
              ].map(s => (
                <div key={s.label} style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 12, color: "#888", display: "block", marginBottom: 6, letterSpacing: 1 }}>{s.label.toUpperCase()}</label>
                  <input
                    defaultValue={s.value}
                    disabled={!s.editable}
                    style={{ width: "100%", padding: "10px 14px", border: "1px solid #e0e0e0", borderRadius: 8, fontSize: 14, color: s.editable ? "#1a1a1a" : "#aaa", background: s.editable ? "white" : "#f9f9f9", boxSizing: "border-box", outline: "none" }}
                  />
                </div>
              ))}
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: 16, background: "#f9f9f9", borderRadius: 8 }}>
                ...
                <span style={{ fontSize: 14, color: "#666" }}>Website connection: <strong style={{ color: connected ? "#2e7d32" : "#c62828" }}>{connected ? "Active" : "Disconnected"}</strong></span>
                <button onClick={() => { setConnected(false); setTimeout(() => setConnected(true), 1500); }} style={{ marginLeft: "auto", padding: "6px 14px", background: LIGHT_PINK, border: `1px solid ${PINK}44`, borderRadius: 6, color: PINK, fontSize: 12, fontWeight: "bold", cursor: "pointer" }}>Test Connection</button>
              </div>
            </div>
          )}
        </div>
      ...
      {showAddModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "white", borderRadius: 16, padding: 32, width: 460, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize: 18, fontWeight: "bold", marginBottom: 24, color: "#1a1a1a" }}>Add New Product</div>
            {[
              { label: "PRODUCT NAME", key: "name", type: "text", placeholder: "e.g. Glow Booster Cream" },
              { label: "PRICE (₦)", key: "price", type: "number", placeholder: "e.g. 15000" },
              { label: "DESCRIPTION", key: "description", type: "text", placeholder: "Short product description" },
            ].map(field => (
              <div key={field.key} style={{ marginBottom: 16 }}>
                <label style={{ fontSize: 11, color: "#888", display: "block", marginBottom: 6, letterSpacing: 1 }}>{field.label}</label>
                <input
                  type={field.type}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 11, color: "#888", display: "block", marginBottom: 6, letterSpacing: 1 }}>CATEGORY</label>
              ...
              <div>
                <label style={{ fontSize: 11, color: "#888", display: "block", marginBottom: 6, letterSpacing: 1 }}>STOCK STATUS</label>
            ...
            <div style={{ padding: "20px", border: "2px dashed #e0e0e0", borderRadius: 8, textAlign: "center", marginBottom: 20, color: "#aaa", fontSize: 13 }}>
              🖼️ Click to upload product image
            </div>
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button onClick={handleAddProduct} style={{ flex: 1, padding: 12, background: PINK, border: "none", borderRadius: 8, fontSize: 14, fontWeight: "bold", cursor: "pointer", color: "white" }}>Add to Website</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "white", borderRadius: 16, padding: 32, width: 460, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            {[
              ...
              <div key={field.key} style={{ marginBottom: 16 }}>
                ...
                <input
                  ...
                  onChange={e => setSelectedProduct(p => ({ ...p, [field.key]: field.type === "number" ? parseInt(e.target.value) || 0 : e.target.value }))}
                  style={{ width: "100%", padding: "10px 14px", border: "1px solid #e0e0e0", borderRadius: 8, fontSize: 14, outline: "none", boxSizing: "border-box" }}
                />
              </div>
            ))}
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button onClick={handleUpdateProduct} style={{ flex: 1, padding: 12, background: PINK, border: "none", borderRadius: 8, fontSize: 14, fontWeight: "bold", cursor: "pointer", color: "white" }}>Save Changes</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showDeleteConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "white", borderRadius: 16, padding: 32, width: 360, textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize: 40, marginBottom: 16 }}>🗑️</div>
            <div style={{ fontSize: 18, fontWeight: "bold", marginBottom: 8, color: "#1a1a1a" }}>Delete Product?</div>
            <div style={{ fontSize: 14, color: "#888", marginBottom: 24 }}>This will permanently remove the product from your website. This cannot be undone.</div>
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button onClick={() => handleDelete(showDeleteConfirm)} style={{ flex: 1, padding: 12, background: "#c62828", border: "none", borderRadius: 8, fontSize: 14, fontWeight: "bold", cursor: "pointer", color: "white" }}>Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
